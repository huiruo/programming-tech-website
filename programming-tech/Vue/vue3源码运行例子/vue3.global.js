var Vue = (function (exports) {
      ...
      switch (type) {
        ...
        default:
          if (shapeFlag & 1 /* ShapeFlags.ELEMENT */) {
            ...
            console.log(`%chydrateNode:2调用mountComponent:`, 'color:magenta')
    ...
    if (!instance.render) {
      ...
      if (!isSSR && compile && !Component.render) {
        ...
        if (template) {
          ...
          console.log('finishComponentSetup=>compileToFunction被调用:', { instance, template, finalCompilerOptions })
          // debugger
          Component.render = compile(template, finalCompilerOptions);
          console.log('finishComponentSetup=>compileToFunction被调用返回值:', { render: Component.render })
          {
  ...
  */
  ...
  function compileToFunction(template, options) {
    console.log('compileToFunction被调用-开始:', { template, options })
    ...
    console.log('探究初始化:compileToFunction被调用==>compile$1：', { template: template })
    ...
    render._rc = true;
    console.log('%c探究初始化结束:compileToFunction被调用==>生成由AST生成的code转化的render函数：', "color:yellow", { render: render })
    console.log('%c探究初始化结束:compileToFunction被调用==>render放入缓存：', "color:yellow")
    return (compileCache[key] = render);
  }

  console.log('探究初始化==>将编译函数注册到运行时', compileToFunction)
